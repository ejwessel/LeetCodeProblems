...
class Solution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        b_word_combo = defaultdict(list)
        e_word_combo = defaultdict(list)

        for word in wordList:
            for i in range(len(word)):
                key = word[:i] + '*' + word[i + 1:]
                b_word_combo[key].append(word)
                e_word_combo[key].append(word)
        ...
        while b_q and e_q:
            ...
            visited[b_current] = b_depth
            visited[e_current] = e_depth
